...
func listenersFromVirtualHosts(externalVirtualHosts []*route.VirtualHost,
	...
	externalManager := kourierEnvoy.NewHTTPConnectionManager(externalVirtualHosts)
	internalManager := kourierEnvoy.NewHTTPConnectionManager(clusterLocalVirtualHosts)
	...
	// names.
	internalRDSHTTPConnectionManager := kourierEnvoy.NewRDSHTTPConnectionManager(internalRouteConfigName)
	internalManager.RouteSpecifier = &internalRDSHTTPConnectionManager

	// Set the discovery to ADS
	externalRDSHTTPConnectionManager := kourierEnvoy.NewRDSHTTPConnectionManager(externalRouteConfigName)
...
func newExternalEnvoyListenerWithOneCert(manager *httpconnmanagerv2.HttpConnectionManager, kubeClient kubeclient.Interface) (*v2.Listener, error) {
	...
	return kourierEnvoy.NewHTTPSListener(manager, config.HTTPSPortExternal, certificateChain, privateKey)
}

func newExternalHTTPEnvoyListener(manager *httpconnmanagerv2.HttpConnectionManager) (*v2.Listener, error) {
	return kourierEnvoy.NewHTTPListener(manager, config.HTTPPortExternal)
}

func newInternalEnvoyListener(manager *httpconnmanagerv2.HttpConnectionManager) (*v2.Listener, error) {
	return kourierEnvoy.NewHTTPListener(manager, config.HTTPPortInternal)
}

func newExternalHTTPSEnvoyListener(manager *httpconnmanagerv2.HttpConnectionManager, sniMatches []*kourierEnvoy.SNIMatch) (*v2.Listener, error) {
	return kourierEnvoy.NewHTTPSListenerWithSNI(manager, config.HTTPSPortExternal, sniMatches)
}
